...
async function testOCR() {
  ...
  try {
    ...
    document?.entities?.forEach((entity) => {
      console.log(`- ${entity.type}: ${entity.mentionText} (confidence: ${entity.confidence})`);
    });

    const totalAmount = document?.entities?.find(e => e.type === 'total_amount');
    const receiptDate = document?.entities?.find(e => e.type === 'receipt_date');
...
async function testLineMessage() {
  ...
  try {
    ...
    console.error('❌ LINE Message Test Failed:', error.response?.data || error.message);
    process.exit(1);
  }
}
